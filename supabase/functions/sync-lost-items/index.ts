...
console.log('Hello from Lost Items Sync Function!');
...
/**
 * 공공 API에서 분실물 데이터를 가져오는 함수
 * @param apiKey - 공공 데이터 포털 API 키
 * @param pageNo - 페이지 번호
 * @param numOfRows - 한 페이지당 결과 수
 * @returns 분실물 항목 배열
 */
async function fetchLostListItems(
  apiKey: string,
  pageNo = 1,
  numOfRows = 100
): Promise<LostListItem[]> {
  try {
    // 공공 데이터 포털 API URL 생성
    const url = `https://apis.data.go.kr/1320000/LostGoodsInfoInqireService/getLostGoodsInfoAccToClAreaPd?serviceKey=${apiKey}&pageNo=${pageNo}&numOfRows=${numOfRows}`;
...
serve(async (req) => {
  ...
  try {
    ...
    // 공공 API에서 데이터 가져오기
    const items = await fetchLostListItems(apiKey);
    ...
    // Supabase에 데이터 저장
    const { error } = await supabaseClient.from('lost_list').upsert(
      items.map((item) => ({
        atc_id: item.atcId,
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/sync-lost-items' \
