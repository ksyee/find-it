...
/**
 * Supabase에서 습득물 데이터를 가져오는 함수
 * @param options 페이지네이션, 검색 옵션
 * @returns 습득물 데이터 배열
 */
export async function getFindItems(
  options: {
    pageNo?: number;
    numOfRows?: number;
    category?: string;
  ...
  try {
    ...
    console.log(
      `Supabase에서 습득물 데이터 조회 (페이지: ${pageNo}, 행 수: ${numOfRows})`
    );
    ...
    if (!data || data.length === 0) {
      console.log('Supabase에 데이터가 없습니다. 동기화를 시도합니다...');

      // 데이터 동기화 시도
      const syncResult = await syncFindItems(100);
      ...
      if (syncResult.success) {
        ...
        if (refreshError) {
          console.error('동기화 후 데이터 조회 오류:', refreshError);
          throw refreshError;
        }

        if (refreshedData && refreshedData.length > 0) {
          console.log(
            `동기화 후 Supabase에서 ${refreshedData.length}개 항목 조회 성공`
          );
          return transformToAllData(refreshedData);
        }
      }

      // 동기화 실패 또는 동기화 후에도 데이터가 없는 경우 API에서 직접 가져오기
      console.log(
        'Supabase 동기화 후에도 데이터가 없거나 오류가 발생했습니다. API에서 직접 가져옵니다...'
      );
    ...
    console.log(`Supabase에서 ${data.length}개 항목 조회 성공`);
    return transformToAllData(data);
  } catch (error) {
 ...
 */
function transformToAllData(data: any[]): AllData[] {
  return data.map((item) => ({
    atcId: item.atc_id,
    fdPrdtNm: item.item_name,
    fdFilePathImg: item.image,
    fdYmd: item.date,
    depPlace: item.place,
    fdSbjt: item.content,
    fdSn: item.mgmt_num,
...
export async function checkAndSyncData() {
  try {
    ...
    if (!data) {
      ...
      console.log('동기화 기록이 없습니다. 최초 동기화를 시작합니다...');
      return await syncFindItems(100);
    }
    ...
    if (diffHours > 2 || data.status !== 'success') {
      console.log(
        `마지막 동기화로부터 ${diffHours.toFixed(1)}시간 경과. 동기화를 시작합니다...`
      );
      return await syncFindItems(100);
    }

    console.log(
      `마지막 동기화: ${lastSync.toLocaleString()} (${diffHours.toFixed(1)}시간 전)`
    );
